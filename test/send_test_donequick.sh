...
while [  $COUNTER -lt 4 ]; do
    ...
    if [ "$?" -ne "0" ]; then
        echo "Sorry, system didn't get exit status for state of test message properly"
        exit 1
    fi

    stat_result=(${out3})
    if [ "${stat_result[0]}" != "ok" ]; then
        echo "Sorry, system didn't get state of message properly : ${out3}"
        exit 1
    fi

    if [ "${out3}" == "ok done" ]; then
